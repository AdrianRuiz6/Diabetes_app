...
public class ColorCustomization : MonoBehaviour
{
    ...
    void onButtonClick()
    {
        if(coinsManager.total_coins >= sellingPrice && acquired == false) // Si el usuario no tiene en posesión el color y tiene dinero para comprarlo.
        {
            coinsManager.subtractCoins(sellingPrice);
            acquired = true;
        }else if(coinsManager.total_coins < sellingPrice && acquired == false) // Si el usuario no tiene en posesión el color y NO tiene dinero para comprarlo.
        {
