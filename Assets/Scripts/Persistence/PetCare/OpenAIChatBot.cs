...
namespace Master.Persistence.PetCare
{
    public class OpenAIChatBot : IChatBot
    {
        ...
        private readonly ChatCompletionsRequest _chatCompletionsRequest;
        ...
        public OpenAIChatBot(string apiKey)
        {
            ...
            _chatCompletionsApi.ConversationHistoryMemory = 10;
            _chatCompletionsApi.SetSystemMessage(initialSystemText);

            _chatCompletionsRequest = new ChatCompletionsRequest
            {
                Model = "gpt-4o",
                Temperature = 0,
                MaxTokens = 50,
        ...
        public async Task<string> AskAsync(string input)
        {
            ...
            try
            {
                _chatCompletionsRequest.AddMessage(new Message(Roles.USER, input));
                ChatCompletionsResponse response = await _chatCompletionsApi.CreateChatCompletionsRequest(_chatCompletionsRequest);
